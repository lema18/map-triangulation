...
void fill_new_3d_points(int map_vertex,cv::Point3d pt,tracking_store &obj)
{
 ...
 void add_new_points_proyections(vector<KeyPoint> _features1,vector<KeyPoint> _features2,vector<int> _left_matches,vector<int> _right_matches,int &_point_identifier,int last_frame,int current_frame,Mat used_features,tracking_store &obj)
    ...
    void initial_guess_for_3d_points(int total_points,vector<int> valid_points,Eigen::Vector2d principal_point,double focal_length,tracking_store &obj)
    {
        for(int i=0;i<total_points;i++)
        {
            auto search_point=obj.pt_2d.find(i);
            if(search_point!=obj.pt_2d.end())
            {
                if(valid_points[i]==1)
                {
                    ...
                    double  z=1.5; //initial z invented
...
void initial_map_generation(tracking_store &obj,string groundtruth_path,int nImages,int dense,int robust_kernel,int img_threshold,int &ident,double focal_length,Eigen::Vector2d principal_point,vector<int> &valid_points,vector<int> &keyframes,int niter)
{
    ...
    initial_guess_for_3d_points(ident,valid_points,principal_point,focal_length,obj);
	...
	for (int i = 0; i < keyframes.size(); i++)
    {
        ...
        cout<< "cam_pos = "<<endl<<" "<< cam_pos_cv <<endl << endl;
...
void matchFeatures_and_compute_essential(int last_frame,int current_frame,vector<KeyPoint> _features1,vector<KeyPoint> _features2,
...
void estimate_motion_and_calculate_3d_points(int last_frame,int current_frame,cv::Mat &img1,cv::Mat &img2,
                                            ...
                                            double scale,cv::Mat &mask,vector<Point3d> &pts3d,tracking_store &obj,vector<int> &valid_points)
{
    ...
    cout<< "cam_pos = "<<endl<<" "<< new_camera_pose <<endl << endl;
    for(unsigned int i=0;i<used_features.size();i++)
    {
